...
/*Blah Blah websites used to help http://www.elook.org/programming/c/strcmp.html
 * http://stackoverflow.com/questions/7758361/c-program-to-read-write-text-file
 * http://www.codingunit.com/c-tutorial-file-io-using-text-files
 * http://cboard.cprogramming.com/c-programming/104967-stopwatch-timer-simple-c-program.html
 * http://www.velocityreviews.com/forums/t318260-peek-at-stdin-flush-stdin.html
 * */
 
 
/*Prototyping Functions*/

int additionFlashcards(int *a, int *b);
void highScoreDisplay();
void menuFunction();
int multiplicationFlashcards(int *a, int *b);
...
int main ()
{
    ...
    int correctAnswer;
    int currentScore;
    int flashCardNumberOne;
    int flashCardNumberTwo;
    int garbageInt;
    ...
    correctAnswer = 0;
    currentScore = 0;
    flashCardNumberOne = 0;
    flashCardNumberTwo = 0;
    garbageInt = 0;
        ...
        if (menuInput < 2)
        {
            ...
            while (k < numberOfRounds)
            {
                ...
                switch(menuInput)
                {
                    ...
                    case 0:
                    correctAnswer = multiplicationFlashcards( &flashCardNumberOne, &flashCardNumberTwo);
                    break;
                    
                    case 1:
                    correctAnswer = additionFlashcards( &flashCardNumberOne, &flashCardNumberTwo);
                    break;
					...
					else
					{
						...
						else
						{
							...
							switch(menuInput)
							{
								/*Will get the correct answer based on which type of flashards they want*/
								case 0:
								garbageInt = multiplicationFlashcards( &flashCardNumberOne, &flashCardNumberTwo);
								printf("What is %d * %d?", flashCardNumberOne, flashCardNumberTwo);
								break;
								
								case 1:
								garbageInt = additionFlashcards( &flashCardNumberOne, &flashCardNumberTwo);
								printf("What is %d + %d? ", flashCardNumberOne, flashCardNumberTwo);
								break;
			...
			if (midGameQuit == 0)
			{
				...
				if (currentScore == k)
				{
					...
					/*Beginning the comparing of the score the user just got and 
					 * what the high score list laready is
					 * 
					 * 
					 * 
					 * 
					...
					/*This next segment will rewrite the tet file, based on the new order of the
...
int additionFlashcards(int *a, int *b)
{
	...
	printf("%d hi %d", randomNumberOne, randomNumberTwo);
	*a *= randomNumberOne;
	*b *= randomNumberTwo;
